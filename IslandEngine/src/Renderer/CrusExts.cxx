...
__forceinline OpenGLContext &OpenGLContext::ThreadLocal()
{
...
void OpenGLContext::SetupContext()
{
    ...
    //glDebugMessageCallback(Render::DebugCallback, nullptr);

    if (glGetError() != GL_NO_ERROR) {
        isle::log::Error() << __LINE__;
        return;
    }
}
...
void OpenGLContext::DeleteContext()
{
    ...
    if (hRC != nullptr) {
        ...
        else if (wglMakeCurrent(nullptr, nullptr) == TRUE)
            hSharedRCs_.push(hRC);
    }
}

#define GL_PROC_INIT(name)\
    thread_local static auto name = OpenGLContext::ThreadLocal().GetProcedureAddress<decltype(::name)>(#name);
